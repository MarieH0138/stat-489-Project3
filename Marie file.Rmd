...
Prediction timeeee haha jk, making model time
...
model1_preds_error <- model1_preds %>%
  mutate(error = sqrt((pred_x - x_last)^2 + (pred_y - y_last)^2),
    accurate = error < 5
           )


#want to collapse the double matrix of prediction into one column or something, thinking thinking ... not working atm
#test <- model1_preds %>%
 # mutate(prediction = lapply(pred, as.numeric)) %>%
